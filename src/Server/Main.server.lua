...
local function BuildLobby()
	...
	-- ===== POTION STAND (near campfire) =====
	local potionStand = mp({Name="PotionStand", Size=Vector3.new(5, 3, 3),
		Position=Vector3.new(8, 2.2, 16),
		Material=Enum.Material.Wood, BrickColor=BrickColor.new("Dark orange")})

	-- Potion bottles on the table
	local potionColors = {
		Color3.fromRGB(220, 50, 50),   -- Health (red)
		Color3.fromRGB(50, 100, 255),  -- Mana (blue)
		Color3.fromRGB(50, 220, 80),   -- Speed (green)
		Color3.fromRGB(255, 160, 30),  -- Strength (orange)
	}
	for i, color in ipairs(potionColors) do
		local bx = 6.2 + (i - 1) * 1.2
		local bottle = mp({Name="PotionBottle"..i, Size=Vector3.new(0.8, 1.4, 0.8),
			Position=Vector3.new(bx, 4.4, 16),
			Material=Enum.Material.Glass, Color=color,
	...
	mp({Size=Vector3.new(2, 4, 2), Position=Vector3.new(8, 5.7, 16),
		Material=Enum.Material.Wood, BrickColor=BrickColor.new("Dark orange")})
	makeSign(
		Vector3.new(8, 8.5, 16), Vector3.new(8, 3, 0.5), Enum.NormalId.Front,
		"POTIONS", Color3.fromRGB(180, 100, 255),
		"Approach to browse", Color3.fromRGB(200, 200, 200)
	)

	-- Proximity prompt to open shop
	local shopPrompt = Instance.new("ProximityPrompt")
	shopPrompt.ObjectText = "Potion Stand"
	shopPrompt.ActionText = "Browse Potions"
	shopPrompt.MaxActivationDistance = 12
	shopPrompt.HoldDuration = 0
	shopPrompt.RequiresLineOfSight = false
	shopPrompt.Parent = potionStand
	shopPrompt.Triggered:Connect(function(player)
		local remote = Remotes:GetEvent("OpenPotionShop")
		if remote then
	...
	-- ===== TREES (deferred — decorative, loads in background) =====
	task.defer(function()
	local treePositions = {
	...
	end) -- end deferred trees

	-- ===== ROCKS (deferred — decorative) =====
	task.defer(function()
	local rockPositions = {
		-- Near cave
	...
	-- ===== MASSIVE RUN-DOWN COLOSSEUM (deferred — heavy decorative geometry) =====
	task.defer(function()
